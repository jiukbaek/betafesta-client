...
nhn.husky.SE_QuickEditor_Image = jindo.$Class({
  ...
  $ON_MSG_APP_READY: function () {
    ...
    this.nMaxHeight =
      nhn.husky.SE2M_Configuration.QuickEditor.Image.nImageMaxHeightSize || Infinity;
  ...
  _assignHTMLObjects: function (elAppContainer) {
    ...
    this.elPanelBGPaletteHolder = jindo.$$.getSingle(
      ".husky_se2m_img_qe_bg_paletteHolder",
      this.elQELayer
    );
    ...
    this.elCheckImgAutoAdjust = jindo.$$.getSingle(
      ".se2_srate",
      this.elQELayer
    ); /* 이미지의 가로/세로 길이를 따로따로 설정할 수 있도록 함 */
  ...
  $LOCAL_BEFORE_FIRST: function (sMsg) {
    ...
    this.oApp.registerBrowserEvent(
      this.elPanelImgWidth,
      "mousedown",
      "IMG_QE_RESIZE_START_PALETTE"
    );
    this.oApp.registerBrowserEvent(
      this.elPanelImgHeight,
      "mousedown",
      "IMG_QE_RESIZE_START_PALETTE"
    );
  ...
  $ON_HIGH_EDIT_CLICK: function () {
    if (
      !!this.welElement &&
      this._hasThumbnail(this.elElement.id) &&
      this.welElement.attr("s_subtype") == "photo"
    ) {
  ...
  _resizeImage: function (elEvent) {
    ...
    if (elCurrentFocus.isEqual(this.elPanelImgWidth)) {
      ...
      if (this.nMaxWidth > nImgAfterWidth && nImgAfterWidth > 0) {
        ...
        if (this.bImgAutoAdjust) {
          /* 가로/세로 비율 유지하면서 사이즈 조절 */
  ...
  _adjustViewImageSize: function (htSize) {
    ...
    if (welWysiwygBody) {
      ...
      if (htSize.nAdjustWidth > nEditorWidth) {
        ...
        var msg = this.oApp
          .$MSG("SE_QuickEditor_Image.exceedMaxSize")
          .replace("${nEditorWidth}", nEditorWidth);
        alert(msg);
      }
    }
    return htImageSize;
  ...
  $ON_IMG_QE_TOGGLE_BGC_PALETTE: function () {
    // border BGC 버튼 토글
  ...
  $ON_IMG_QE_SET_BORDER: function (nBorderSize) {
    // border size
  ...
  $ON_IMG_QE_SET_BGC_FROM_PALETTE: function (sColorCode) {
    // border color
