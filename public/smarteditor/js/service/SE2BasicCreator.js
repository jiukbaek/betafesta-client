...
window.createSEditor2 = function (elIRField, htParams, elSeAppContainer) {
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE_EditingAreaManager(
      "WYSIWYG",
      oIRTextarea,
      htDimension,
      htParams.fOnBeforeUnload,
      elAppContainer
    )
  );
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE_EditingAreaVerticalResizer(elAppContainer, htConversionMode)
  ); // 편집영역 리사이즈
  ...
  oEditor.registerPlugin(
    new nhn.husky.SE2M_FontNameWithLayerUI(elAppContainer, aAdditionalFontList)
  ); // 글꼴종류
